import DiffMatchPatch from 'googlediff';
...
function cledit(contentElt, scrollEltOpt, isMarkdown = false) {
  ...
  function isScrollNavigationKey(evt) {
    return !evt.altKey && !evt.ctrlKey && !evt.metaKey && scrollNavigationKeys.has(evt.key);
  }
  ...
  windowKeydownListener = (evt) => {
    if (!tryDestroy()) {
      if (evt.target && evt.target.nodeType && contentElt.contains(evt.target)) {
        return;
      }
