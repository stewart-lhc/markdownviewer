...
| Share Markdown properly | Create short stored `/share/{id}` links instead of pushing large Markdown payloads into the URL. |
...
| 正式分享 Markdown | 创建短的 `/share/{id}` 存储链接，而不是把大段 Markdown 塞进 URL。 |
| 管理一组文档 | 使用持久 workspace tabs、文件夹浏览、相对 Markdown link 跳转，以及支持浏览器里的保存到磁盘。 |
